...
contract RDS is AuthBase {
  ...
  string public constant name = "RealDAO governance token";
  ...
  modifier onlySuperior {
    require(superior == msg.sender, "RDS/permission denied");
    _;
  }
  ...
  function approve(address spender, uint256 amount) external returns (bool) {
    ...
    require(spender != address(0), "RDS/approve to zero address");
  ...
  function transferFrom(
    ...
    require(balances[src] >= amount, "RDS/insufficient-balance");
    require(src != address(0), "RDS/transfer from zero address");
    require(dst != address(0), "RDS/transfer to zero address");

    address sender = msg.sender;
    uint256 allowed = allowances[src][sender];
    if (sender != src && allowed != uint256(-1)) {
      require(allowed >= amount, "RDS/insufficient-allowance");
  ...
  function mint(address src, uint256 amount) external onlySuperior {
    require(totalSupply.add(amount) <= maxSupply, "RDS/max supply exceeded");
    require(src != address(0), "RDS/mint to zero address");
  ...
  function burn(address src, uint256 amount) external {
    require(balances[src] >= amount, "RDS/insufficient-balance");
    require(src != address(0), "RDS/burn from zero address");
  ...
  function delegateBySig(
    ...
    require(signatory != address(0), "RDS/ invalid signature");
    require(nonce == nonces[signatory]++, "RDS/ invalid nonce");
    require(now <= expiry, "RDS/signature expired");
  ...
  function getPriorVotes(address account, uint256 blockNumber) public view returns (uint256) {
    require(blockNumber < block.number, "RDS/not yet determined");
  ...
  function _writeCheckpoint(
    ...
    uint32 blockNumber = safe32(block.number, "RDS/Block number overflow");
